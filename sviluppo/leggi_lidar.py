...
source = "../dati/lidar/lidar.las"
target = "../dati/lidar/lidar.asc"
target_img = "../dati/lidar/lidar.bmp"
...
#converti_las_asc(source,target)


def converti_asc_image(source_sec,target_img):
    arr = np.loadtxt(source_sec,skiprows=6)
    im = Image.fromarray(arr).convert('RGB')
    im = ImageOps.equalize(im)
    im = ImageOps.autocontrast(im)
    im.save(target_img)

converti_asc_image(target,target_img)